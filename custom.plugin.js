...
import defaultTheme, { fontSize } from 'tailwindcss/defaultTheme'
import { gray, green } from 'tailwindcss/colors'
const [baseFontSize, { lineHeight: baseLineHeight }] = fontSize.base
...
const forms = withOptions(function (options = { strategy: undefined }) {
  return function ({ addBase, addComponents, theme }) {
    ...
    const rules = [{
      ...
      styles: {
        ...
        'border-color': resolveColor(theme('colors.gray.500', gray[500]), '--tw-border-opacity'),
        ...
        '&:focus': {
          ...
          'border-color': resolveColor(theme('colors.green.600', green[600]), '--tw-border-opacity'),
        },
      },
    }, {
      ...
      styles: {
        color: resolveColor(theme('colors.gray.500', gray[500]), '--tw-text-opacity'), opacity: '1',
      },
    }, {
      ...
      base: ['select'], class: ['.form-select'], styles: {
        'background-image': `url("${svgToDataUri(`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"><path stroke="${resolveColor(theme('colors.gray.500', gray[500]), '--tw-stroke-opacity')}" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6 8l4 4 4-4"/></svg>`)}")`,
      ...
      base: [`[type='checkbox']`, `[type='radio']`], class: ['.form-checkbox', '.form-radio'], styles: {
        ...
        height: spacing[4],
        width: spacing[4],
        color: resolveColor(theme('colors.primary.600', green[600]), '--tw-text-opacity'),
        'background-color': '#fff',
        'border-color': resolveColor(theme('colors.gray.500', green[500]), '--tw-border-opacity'),
        'border-width': borderWidth['DEFAULT'],
        '--tw-shadow': '0 0 #0000',
      },
    }, {
